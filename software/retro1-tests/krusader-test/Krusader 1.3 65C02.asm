...
.NEXT	LDA MSG-1,X
	...
	;TXS		; reset stack pointer on startup STACHE COMMENTED
...
.LOOP	ASL RMNE
	...
	LDY PRFLAG
	BEQ .SKIP
	JSR ECHO	; print the mnemonic as well
.SKIP	INX
	CPX #$03
	BNE .NEXT
	RTS	
...
.DSMBL2	JSR INSTDSP	; Disassemble and display instr.
	...
	;LDA KBDRDY	; Now disassemble until key press STACHE COMMENTED
	.if APPLE1
	BPL .DSMBL2
	.else
	BEQ .DSMBL2
	.endif
	;LDA KBD STACHE COMMENTED
...
.PPADR4	DEY
	BMI .PPADR2
	JSR PRBYTE	; Output 1- or 2-byte address STACHE COMMENTED
.PPADR5	LDA FORMAT
...
.RELADR	JSR PCADJ3	; PCL,H + DISPL + 1 to A,Y
	...
	INY   
PRNTYX	TYA   
PRNTAX	JSR PRBYTE	; Print target adr of branch STACHE COMMENTED
PRNTX	TXA   		; *  and return
	JMP PRBYTE ;STACHE COMMENTED
...
RESET:      ;CLD             ;Clear decimal arithmetic mode.
            ...
            ;JMP INITRAM     ;init all RAM TO 00
RESET2:
            ;LDA #$1F        ;* Init ACIA to 19200 Baud.
            ;STA ACIA_CTRL
            ;LDA #$0B        ;* No Parity.
         ...
         JSR ECHO      ;* New line.
         LDA #<MSG1
         STA MSGL      ;store to zero page register
         LDA #>MSG1
         STA MSGH
...
SETMODE:     STA MONMODE        ;$00 = XAM, $7B = STOR, $AE = BLOK XAM.
...
NEXTITEM:    LDA IN,Y        ;Get character.
         ...
         CMP #$D2        ;"R"?
            BEQ RUNMON         ;Yes, run user program.
            CMP #$CC        ;* "L"?
            BEQ LOADINT     ;* Yes, Load Intel Code.
            STX L           ;$00->L.
            STX H           ; and H.
            STY MONYSAV        ;Save Y for comparison.
...
NOTHEX:      CPY MONYSAV        ;Check if L, H empty (no hex digits).
         BNE NOESCAPE   ;* Branch out of range, had to improvise...
            JMP ESCAPE      ;Yes, generate ESC sequence.

RUNMON:         JSR ACTRUN      ;* JSR to the Address we want to run.
...
NOESCAPE:    BIT MONMODE        ;Test MODE byte.
...
PRDATA:      LDA #$A0        ;Blank.
            ...
            JSR PRBYTE      ;Output it in hex format.
XAMNEXT:     STX MONMODE        ;0-> MODE (XAM mode).
            LDA XAML
...
zpptr = $00 
	...
	LDY #$00 ;msb start
         ...
         ldy #0                ;index
;
CLEAR: sta (zpptr),y         ;clear
         iny
         bne CLEAR         ;next location
;
         dec pagectr           ;one less page to clear
         beq INITDONE          ;done
;
         inc zpptr+1           ;next page
         bne CLEAR          ;more pages remaining
;
INITDONE:
	LDA #$00 ;clear last zp bytes
	STA $00
	STA $01
	STA $02
	STA $03
...
MSG1:      .byte $0a, $0a, "Welcome to EWOZ 1.0.", $0a, $00  ;!TEXT "Welcome to EWOZ 1.0.",0
MSG2:      .byte $0a, $0a, "Start Intel Hex code Transfer.", $0a, $00  ;!TEXT "Start Intel Hex code Transfer.",0
MSG3:      .byte $0a, $0a, "Intel Hex Imported OK.", $0a, $00  ;!TEXT "Intel Hex Imported OK.",0
MSG4:      .BYTE $0a, $0a, "Intel Hex Imported with checksum error.", $0a, $00  ;!TEXT "Intel Hex Imported with checksum error.",0
